...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        if not API_KEY:
            error_msg = "API_KEY environment variable not set"
            logger.error(error_msg)
            return {"answer": "API key not set.", "links": []}
        ...
        try:
            ...
            logger.info("Enriching with adjacent chunks")
            ...
            answer = result.get("answer", "The assistant could not generate a response.")
            links = result.get("links", [])

            # Fallback link generation if links empty
            if not links:
                logger.info("Generating fallback links")
                unique_urls = set()
            ...
            logger.error(traceback.format_exc())
            return {"answer": "There was an error processing your question.", "links": []}
        finally:
            conn.close()
